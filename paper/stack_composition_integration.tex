...
%-----------------------------------------------------------------------------
\subsubsection{Infrastructure Primitives}
\label{mech:infrastructure-primitives}

\noindent\depthinfo{N/A (architectural)} | \primaryimpl{Architecture} | \litnames{architectural primitives, foundational components}

\begin{functiondesc}
Architectural elements that enable computational mechanisms but are not mechanisms themselves. These are universal requirements for transformer operation: (1) \textbf{Residual Connections} - additive skip connections enabling gradient flow and cross-layer circuits, (2) \textbf{Layer Normalization} - activation stabilization applied before sublayers, (3) \textbf{Attention Masking} - causal masking preventing future token access, (4) \textbf{Embedding/Unembedding} - token$\leftrightarrow$vector conversion, (5) \textbf{Attention Computation} - scaled dot-product attention operation, (6) \textbf{Gradient Flow} - backpropagation infrastructure, (7) \textbf{Tokenization} - text$\leftrightarrow$token conversion (BPE), (8) \textbf{Positional Encoding} - sequence order information.
...
\begin{implementationbox}
\archimpl{Universal architectural requirements present in all transformer models. Residual stream composition: $h_{l+1} = h_l + \text{Attn}(h_l) + \text{MLP}(h_l + \text{Attn}(h_l))$. Pre-LN: normalization applied before sublayers. Causal masking: $M_{ij} = 0$ if $j \leq i$ else $-\infty$. Details in foundational architecture papers.}
\end{implementationbox}

\begin{ablationbox}
\textbf{Expected ablation:} Catastrophic architectural failures. Removing any primitive fundamentally breaks transformer operation: no residual connections = training impossible beyond 3-5 layers; no layer norm = severe instability; no attention masking = autoregressive generation failure; no embeddings = cannot process tokens; no gradient flow = cannot train.
\end{ablationbox}

\begin{examplebox}
These are prerequisites for all transformer computation, not examples of specific mechanisms. See Vaswani et al. (2017), He et al. (2016), Ba et al. (2016) for architectural details.
\end{examplebox}
