...
\textbf{Evidence Level Note:} Safety mechanisms are primarily characterized through behavioral observation and intervention studies. The multi-stage pipeline (detection $\rightarrow$ enforcement $\rightarrow$ refusal) is inferred from behavioral patterns rather than fully traced circuits. Only Refusal Generation has strong mechanistic evidence (single direction finding~\cite{arditi2024refusal}), achieving Tier 1 status. Other safety mechanisms represent consistent behavioral patterns requiring further mechanistic characterization. This asymmetry reflects the relative recency of safety mechanism research compared to core transformer computations.
...
\mechfooter{\statusobs}{policy-enforcement, refusal-generation}

\begin{openquestionsbox}
\textbf{Circuit-level characterization needed:} Detection mechanism inferred from behavioral patterns and ablation studies. Detailed tracing of how detection signals propagate through layers and integrate with enforcement mechanisms remains incomplete. Future research should identify specific components and trace information flow through the safety pipeline.
...
\mechfooter{\statusobs}{harmful-content-detection, refusal-generation, redirection-alternatives}
